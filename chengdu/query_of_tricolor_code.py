...
class Query_Of_Tricolor_Code(TaskSet):
      ...
      def queryInfo(self):
          ...
          personName = "string"
          with self.client.get(loginUrl + "?hashUserCode=" + hashUserCode + "&personName=" + personName, catch_response = True) as response:
              if response.status_code != 200:
                  response.failure("响应失败")
      ...
      def queryUserHealthStatusInfo(self):
          ...
          hashUserCode = "e5089e403ce9873d8e9af3abd5cbde11929d2938c6bdaff6d4255b499877904b"
          personName = "string"
          with self.client.get(loginUrl + "?hashUserCode=" + hashUserCode + "&pserson_name=" + personName, catch_response = True) as response:
              if response.status_code != 200:
                  response.failure("响应失败")
